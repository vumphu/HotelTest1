...
def clear_entry(entry_frame, id_entry, type_entry, price_entry):
    ...
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                       ', font=("Montserrat Bold", 10, 'bold')).grid(column=4,row=3,sticky='w')
...
def room_add(room_list, room_tree, entry_frame, id_entry, type_entry, price_entry):
    ...
    if valid_check == 0:
        ...
        type_entry.delete(0, END)
        price_entry.delete(0, END)
        Label(entry_frame, bg='#5E95FF', fg='crimson', text='                        ', font=("Montserrat Bold", 14, 'bold')).grid(column=4,row=3,sticky='w')
        ...
        for room in room_list:
         for reservation in reservation_list:
            ...
            if reservation.get_RoomID()==room_id:
                  Label(entry_frame, bg='#5E95FF', fg='black', text='Unavailable', font=("Montserrat Bold", 10, 'bold')).grid(column=4,row=3,sticky='w')
                  break
            else:
                  Label(entry_frame, bg='#5E95FF', fg='black', text='Available   ', font=("Montserrat Bold", 10, 'bold')).grid(column=4,row=3,sticky='w')
...
def room_press(window, width, height, room_list, reservation_list):
    global selected_room
    selected_room = -1
    ...
    select_room_button = Button(room_subwin, text='SELECT',anchor='center',font=("Montserrat Bold", 11,'bold'), bg='red',fg='white', relief='ridge',
        activebackground='blue', activeforeground='white', command=lambda: room_select(room_list, reservation_list, room_tree, entry_frame, id_entry, type_entry, price_entry))
