...
def staff_add(staff_list, staff_tree, entry_frame, id_entry, name_entry, gend_entry, dob_entry, phone_entry, email_entry, address_entry, salary_entry, position_entry):
    ...
    if valid_check == 0:
        # Add to staff_list
        new_staff = Staff(id, name, gend, dob, phone, email , address)
        if len(salary) > 0:
        ...
        # Display on Treeview
        staff_tree.insert(parent='', index = 'end', iid=id, text='', values=(id, name, gend, dob))
...
def staff_update(staff_list, staff_tree, entry_frame, id_entry, name_entry, gend_entry, dob_entry, phone_entry, email_entry, address_entry, salary_entry, position_entry):
    ...
    if selected_staff != -1:
        ...
        if valid_check == 0:
            ...
            staff_tree.item(selected_staff, text="", values = (id, name, gend, dob))
            selected_staff = -1
...
def staff_press(window, width, height, staff_list):
    ...
    # Insert Data
    for staff in staff_list:
        staff_tree.insert(parent='', index = 'end', iid=staff.get_id(), text='', values=(staff.get_id(), staff.get_name(), staff.get_gend(), staff.get_dob()))
    ...
    select_staff_button = Button(staff_subwin, text='SELECT',anchor='center',font=("Montserrat Bold", 12,'bold'), bg='#5E95FF',fg='white', relief='ridge',
        activebackground='dark blue', activeforeground='white', command=lambda: staff_select(staff_list, staff_tree, entry_frame, id_entry, name_entry, gend_entry, dob_entry, phone_entry, email_entry, address_entry, salary_entry, position_entry))
