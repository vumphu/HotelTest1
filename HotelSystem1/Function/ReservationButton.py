...
def reservation_add(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry):
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                                  ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=0,sticky='w')
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                   ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=1,sticky='w')
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                   ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=2,sticky='w')
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                   ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=3,sticky='w')
    ...
    if len(rid) == 0:
        ...
        valid_check += 1
    elif utils.invalid_rid(rid, "R-") == 1:
    ...
    # check if new id is different from old id, if yes, check for duplication
            for reservation in reservation_list:
                if reservation.get_rid() == rid:
                    Label(entry_frame, bg='#5E95FF', fg='crimson', text='ID already exist', font=("Times New Roman", 14, 'bold')).grid(column=6,row=1,sticky='w')
                    valid_check += 1
                    break
    ...
    if len(gid) == 0:
        ...
        valid_check += 1
    elif utils.invalid_gid(gid, "G-") == 1:
        Label(entry_frame, bg='#5E95FF', fg='crimson', text='INVALID', font=("Times New Roman", 14, 'bold')).grid(column=6,row=2,sticky='w')
        valid_check += 1
    else:
    # check if new id is different from old id, if yes, check for duplication
            for reservation in reservation_list:
                if reservation.get_gid() == gid:
                    Label(entry_frame, bg='#5E95FF', fg='crimson', text='ID already exist', font=("Times New Roman", 14, 'bold')).grid(column=6,row=2,sticky='w')
                    valid_check += 1
                    break
    ...
    if valid_check == 0:
        # Add to guest_list
        new_reservation= Reservation(no, rid, gid, checkin, checkout, creditcard, paymentdate, billing)
        reservation_list.append(new_reservation)

        # Display on Treeview
        reservation_tree.insert(parent='', index = 'end', iid=id, text='', values=(no, rid, gid, checkin, checkout))
...
def reservation_select(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry):
    # Delete all Warnings
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                                  ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=0,sticky='w')
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                   ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=1,sticky='w')
    Label(entry_frame, bg='#5E95FF', fg='crimson', text='                   ', font=("Times New Roman", 14, 'bold')).grid(column=6,row=2,sticky='w')
...
def reservation_update(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry):
    global selected_reservation
    if selected_reservation!= -1:
        ...
        # check if new id is different from old id, if yes, check for duplication
            if rid != reservation_tree.item(selected_reservation, 'values')[0]:
                for reservation in reservation_list:
                    if reservation.get_rid() == rid:
                        Label(entry_frame, bg='#5E95FF', fg='crimson', text='ID already exist', font=("Times New Roman", 14, 'bold')).grid(column=6,row=1,sticky='w')
                        valid_check += 1
        ...
        # check if new id is different from old id, if yes, check for duplication
            if gid != reservation_tree.item(selected_reservation, 'values')[0]:
                for reservation in reservation_list:
                    if reservation.get_gid() == gid:
                        Label(entry_frame, bg='#5E95FF', fg='crimson', text='ID already exist', font=("Times New Roman", 14, 'bold')).grid(column=6,row=2,sticky='w')
                        valid_check += 1
        ...
        if valid_check == 0:
            ...
            reservation_tree.item(selected_reservation, text="", values = (id, rid, gid, checkin, checkout, paymentdate, creditcard, billing))
            selected_reservation= -1
...
def reservation_press(window, width, height, reservation_list):
    global selected_reservation
    selected_reservation= -1
    ...
    add_reservation_button = Button(reservation_subwin, text='ADD RESERVATION',anchor='center',font=("Times New Roman", 12,'bold'), fg='#5E95FF', relief='ridge',
        activebackground='dark blue', activeforeground='white', command=lambda: reservation_add(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry))
    ...
    update_reservation_button = Button(reservation_subwin, text='UPDATE',anchor='center',font=("Times New Roman", 12,'bold'), fg='#5E95FF', relief='ridge',
        activebackground='dark blue', activeforeground='white', command=lambda: reservation_update(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry))
    ...
    select_reservation_button = Button(reservation_subwin, text='SELECT',anchor='center',font=("Times New Roman", 12,'bold'), bg='#5E95FF',fg='white', relief='ridge',
        activebackground='dark blue', activeforeground='white', command=lambda: reservation_select(reservation_list, reservation_tree, entry_frame, no_entry, rid_entry, gid_entry, checkin_entry, checkout_entry, paymentdate_entry, creditcard_entry, billing_entry))
